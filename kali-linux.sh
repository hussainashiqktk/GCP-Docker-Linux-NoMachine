...
docker exec nomachine-xfce4 bash -c "apt update && apt install -y tor proxychains psmisc python3-pip python3-venv wget curl nmap net-tools && sed -i 's/^socks4.*/socks5 127.0.0.1 9050/' /etc/proxychains.conf 2>/dev/null || sed -i 's/^socks4.*/socks5 127.0.0.1 9050/' /etc/proxychains4.conf && service tor restart && sleep 5 && echo 'alias newip=\"pkill -HUP tor && sleep 5\"' >> /home/user/.bashrc && . /home/user/.bashrc && newip && proxychains curl -s ifconfig.me"
